...
with tabs[1]:
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("PRESS", key="batch_predict_btn"):
                    ...
                    # Save results for audit/history
    ...
    # ------------------------------
        ...
        # Create dynamic fields for PCA features V1–V28
        cols = st.columns(4)
        features_input = {}
        for i in range(1, 29):
            col_idx = (i - 1) % 4
...
#______________________________________live Streaming________________________________________________
with tabs[2]:
...
#---------------------------------explanability______________________________________
# ---------------- SHAP Explainability ----------------
with tabs[3]:
    ...
    if "last_input_df" not in st.session_state:
        st.warning("Run a Developer Mode prediction first.")
    else:
        try:
